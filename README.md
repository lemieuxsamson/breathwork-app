...
├── README.md
├── package.json      # Dépendances de développement (tests uniquement — rien n'est requis en production)
├── tests/            # Suite de tests (node:test) — sécurité, service worker, accessibilité
├── scripts/
...
Le site en production ne dépend d'aucun de ces outils — `index.html` reste un fichier 100 % autonome. `package.json` et `tests/` servent uniquement à valider les changements avant de les déployer.
